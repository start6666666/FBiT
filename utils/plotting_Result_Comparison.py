...
label_map = {
    '改成transformer只接受需求+1个预测网络': 'FBiT(Ours)',
    'sq': 'SQ',
    'ddpg': 'DDPG',
    'stable_line3_env0_sac': 'SAC',
    'stable_line3_env0_crossQ': 'CrossQ',
    'stable_line3_env0_ctd4': 'CTD4'
}
...
def generate_multi_model_plot():
    ...
    for ax_idx, target_model in enumerate(target_models):
        ...
        for idx, (algo_label, (steps, means, stds)) in enumerate(ordered_plot_data):
            color = plt.colormaps[palette](idx / len(ordered_plot_data))
        ...
        # titles = {
        #     'model1': "Simulated data (Simple)",
        #     'model2': "Simulated data (Medium)",
        #     'model5': "Simulated data (Complicated)"
        # }
        ...
        ax.yaxis.set_major_locator(MaxNLocator(integer=True, nbins=6, prune='both'))
    ...
    pdf_path = "multi_Real_model_comparison.pdf"
    png_path = "multi_Real_model_comparison.png"
